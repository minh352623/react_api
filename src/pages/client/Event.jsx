...
const Event = () => {
    ...
    async function renderMenus() {
      ...
      switch (state.menus.active) {
        ...
        case MENU_SCORE:
          ...
          formData.append("coin", state.game.score);
          console.log(JSON.parse(localStorage.getItem("call")));
          if (JSON.parse(localStorage.getItem("call")) == 1) {
            try {
              ...
              if (response) {
                ...
                if (user?.coin_dif) {
                  console.log("hi" + state.game.score);

                  cookies.set("user", {
                    ...user,
                    coin_dif: {
                      ...user.coin_dif,
                      coin: +user.coin_dif.coin + state.game.score,
                    },
                  });
                  dispatch(
                    setUser({
                  ...
                  cookies.set("user", {
                    ...user,
                    coin_dif: { ...user.coin_dif, coin: state.game.score },
                  });
                  dispatch(
                    setUser({
                      ...user,
  ...
  //<div className="event_bg max-h-full h-screen w-full ">ádasd</div>
  return (
    <>
      <canvas id="c"></canvas>
