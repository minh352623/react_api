...
const ListSettings = () => {
  ...
  const [settings, setSeting] = useState([]);
  ...
  const FetchSetting = async () => {
    ...
    try {
      const respone = await axios.get(
        `https://shoppet.site/api/setting/list?query=${query}&page=${page}`,
        {
          headers: { Authorization: "Bearer " + user?.token },
        }
      );
      if (respone.data.data) {
        setSeting(respone.data);
        setLoading(false);
      }
      console.log(respone);
    } catch (err) {
  ...
  useEffect(() => {
    if (!settings || !settings.total) return;

    settings && setPageCount(Math.ceil(settings.total / per_page));
  }, [itemOffset, settings]);
  const handlePageClick = (event) => {
    const newOffset = (event.selected * per_page) % settings.total;
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const data = await axios.delete(
            `https://shoppet.site/api/setting/delete/${id}`,
            {
              headers: { Authorization: "Bearer " + user?.token },
            }
          );
