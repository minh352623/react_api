...
const ListDiscount = () => {
  ...
  const FetchDiscount = async () => {
    ...
    try {
      const respone = await axios.get(
        `https://shoppet.fun/api/discount/list?page=${page}&query=${query}`,
        {
  ...
  const { per_page } = discount || [];
  console.log(per_page);
  useEffect(() => {
    if (!discount || !discount.total) return;

    discount && setPageCount(Math.ceil(discount.total / per_page));
  }, [itemOffset, discount]);
  const handlePageClick = (event) => {
    const newOffset = (event.selected * per_page) % discount.total;
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const data = await axios.delete(
            `https://shoppet.fun/api/discount/delete/${id}`,
            {
              headers: { Authorization: "Bearer " + user?.token },
            }
          );
                  ...
                  discount.data.map((item) => (
                    ...
                    >
                      ...
                      <TableCell align="left">
                        <Link to={"/updateDiscount/" + item.id}>
                          <span>
                            <svg
