...
const ListPartner = () => {
  ...
  const FetchPartner = async () => {
    ...
    try {
      ...
      if (respone.data.data) {
        respone.data.data && setPartners(respone.data);
        setLoading(false);
      }
      console.log(respone);
    } catch (err) {
  ...
  const { per_page } = partners || [];
  console.log(per_page);
  useEffect(() => {
    if (!partners || !partners.total) return;

    partners && setPageCount(Math.ceil(partners.total / per_page));
  }, [itemOffset, partners]);
  const handlePageClick = (event) => {
    const newOffset = (event.selected * per_page) % partners.total;
